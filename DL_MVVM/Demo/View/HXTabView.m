...
- (void)buildUI {
    ...
    [self cleanTabView];
    //build UI
    for (HXTabItem *item in self.tabItems) {
...
- (void)swtichTab:(HXTabItem *)tabItem {
...
- (void)pressedTab:(id)sender {
    ...
    HXTabItem *tabItem = [self tabItemForTab:tabView];
    [self swtichTab:tabItem];
}

#pragma mark - Helper methods

- (UIControl *)existingTabForTabItem:(HXTabItem *)tabItem {
...
- (int)indexOfTab:(HXTabItem *)tabItem {
    return (int)[self.tabItems indexOfObject:tabItem];
}

- (HXTabItem *)tabItemForTab:(UIControl *)tab {
    return self.tabItems[[self.tabViews indexOfObject:tab]];
}

- (CGRect)frameForTab:(HXTabItem *)tabItem {
...
- (void)setTabContent:(UIControl *)tab withTabItem:(HXTabItem *)tabItem {
    ...
    if (tabItem.titleString.length != 0) {
        ...
        titleSize = [tabItem.titleString dl_sizewithfont:titleLabel.font constrainedToSize:CGSizeMake(tab.bounds.size.width, MAXFLOAT)];
    ...
    if (titleLabel) {
        ...
        //        titleLabel.frame = CGRectMake(tab.bounds.size.width/2 - titleSize.width/2, 58, titleSize.width+2, titleSize.height+2);
